...
//   /projects/31092/survey-list(dialog:choose-template)
//   /surveys/d17/questionnaire
//   /surveys/d17/questionnaire(dialog:select-questions)
//   /surveys/d17/questionnaire(dialog:custom-question/c1757)
//
// Production uses real paths; a static prototype on GitHub Pages has no server
// rewrites, so the path lives in the hash (`#/surveys/d17/questionnaire(...)`).
// Everything after `#` is byte-identical to the platform path, which is the part
// that matters when sharing a link in a review.
//
// Kept deliberately in step with phase-2's routes.js: same shape, same dialog
// grammar, so a link from one phase reads like a link from the other.
